...
#splitting and loading the data; so this is comparable with my other model, I am splitting it into the predetermined train and test
#the split between train and validation will be different, but the most important thing is that the test data is the same
...
train_df, val_df = mu.split_data_SEDD(df, using_test=False)
...
semantic_loss_fn = mu.PWCFLNonMask(alpha=0.25).to(device)
...
for epoch in range(num_epochs):
    ...
    train_loss = mu.fit(model, train_loader, optimizer, semantic_loss_fn, distance_loss_fn, device, regularized=True)

    # Validate the model and calculate validation loss
    val_loss = mu.validate(model, val_loader, semantic_loss_fn, distance_loss_fn, device, regularized=True)
    ...
    if (epoch+1) % 5 == 0:
        torch.save(model.state_dict(), f"Regularized_model_{epoch+1}.pth")

model = mu.SEDDModel(num_classes=6).to(device)
model.load_state_dict(torch.load('Regularized_model_15.pth'))
